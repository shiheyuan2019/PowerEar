...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        # basic parameters 
        parser.add_argument('--dataroot', default='./datasets/baidufanyi_2cm',  help='path to images (should have subfolders trainA, trainB, valA, valB, etc)')
        parser.add_argument('--name', type=str, default='baidufanyi_2cm_pix2pix', help='name of the experiment. It decides where to store samples and models')
        parser.add_argument('--gpu_ids', type=str, default='4', help='default change from 1->2,gpu ids: e.g. 0  0,1,2, 0,2. use -1 for CPU')
        ...
        parser.add_argument('--netD', type=str, default='basic_256', help='specify discriminator architecture [basic | n_layers | pixel]. The basic model is a 70x70 PatchGAN. n_layers allows you to specify the layers in the discriminator')
        
        parser.add_argument('--netG', type=str, default='unet_256', help='specify generator architecture [resnet_9blocks | resnet_6blocks | unet_256 | unet_128]') # There are practical restrictions regarding image sizes for each generator architecture. For unet256, it only supports images whose width and height are divisible by 256. For unet128, the width and height need to be divisible by 128. For resnet_6blocks and resnet_9blocks, the width and height need to be divisible by 4.
        ...
        parser.add_argument('--batch_size', type=int, default=32, help='input batch size') # 1。
